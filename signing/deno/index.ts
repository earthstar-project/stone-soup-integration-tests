...
const cryptoTests = (cryptoDriver: ICryptoDriver) => {
    ...
    const msg = "hello";
    const expectedSig =
        "bkd372vxiqdu323dk65tn5s43zr6xpbkk2d7hoxj73pn7f7b4oiyndefybdzjrelnloioqoy2n2zh6gfymhy6drh6d42gwwr7fociqda";
    ...
    test(`${cryptoDriverName}: generateAuthorKeypair`, async (t) => {
        ...
        const newKeypair: AuthorKeypair | ValidationError = await Crypto.generateAuthorKeypair(
            "test",
        );
    ...
    test(`${cryptoDriverName}: verify`, async (t) => {
        ...
        const verifiedBad = await Crypto.verify(
            knownKeypair.address,
            expectedSig,
            "some other message",
        );
